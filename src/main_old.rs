                ...
                if ui.button("Enviar").clicked() {
                    if !self.input.is_empty() && (!self.encrypt_key.is_empty() || !self.decrypt_key.is_empty()) {
                        ...
                        if self.encrypt {
                            ...
                            hex::decode_to_slice(encrypt_key_bytes, &mut decoded).expect("Decoding failed");
                            ...
                            let mut decoded: [u8; 16] = [0; 16];
                            hex::decode_to_slice(decrypt_key_bytes, &mut decoded).expect("Decoding failed");
                            let decrypted_data = decrypt(input_data, decoded);
